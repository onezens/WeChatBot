...
// http://iphonedevwiki.net/index.php/Logos
    ...
    #define UNIQUE_SUFFIX(stuff) CONCAT(stuff, __COUNTER__)
        ...
        #define END_SUBCLASS() %end
        #define CTOR() %ctor
        #define NEWPROPERTY(type, strong, name) %property (nonatomic,strong) type * name
        #define NEWPROPERTY_T(type, assign, name) %property (nonatomic,assign) type name
        #define NEWPROPERTY_Block(type, name) %property (nonatomic,copy) type name
